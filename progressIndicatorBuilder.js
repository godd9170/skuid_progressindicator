							...
							props : [
								...
								},
								{
									...
									onChange : function(val) {								
										...
										if (iconelem !== undefined && iconelem.length) {
											iconelem.remove();
										}
			...
			if(state.attr('mode') === undefined || state.attr('mode') == 'tabwiz'){
				...
				skuid.model.load([tempModel]).done(function(){
					$.each(tempModel.getField(state.attr('field')).picklistEntries, function(i, entry){
						var stepEntry = state.attr('step_' + entry.label.replace(/\s/g, '')); //spaceless label is the unique id
						
						if(stepEntry === undefined || (stepEntry === 'false')) { //add the step is exclude == false OR undefined
		...
		propertiesRenderer: function (propertiesObj,component) {
			...
			var picklistProps = [
				...
				},
				{
					...
					onChange: function(){
						component.refresh().rebuildProps(); //rebuild so the exclude shows
					}
				}
			];
			...
			if(component.state.attr('mode') === undefined || component.state.attr('mode') === 'tabwiz'){
				...
				propertiesObj.applyPropsWithCategories(propCategories,state); //do the Apply here syncronously
			}
			else{
				properties = properties.concat(picklistProps); //concat the standard picklist props
				if(component.state.attr('field') !== undefined) { //if there is a picklist, let's make a new property page
					console.log("Field!!: ", component.state.attr('field'));
						// Add the picklist entries as steps
					var excludePicklistProperties = [];
					var tempModel = new skuid.model.Model(skuid.builder.core.getPageXML().find('model#' + state.attr('model')));

					skuid.model.load([tempModel]).done(function(){
						var picklistEntries = !!tempModel.getField(state.attr('field')) ? tempModel.getField(state.attr('field')).picklistEntries : []; //watch for the undefined entires
						console.log("Picklist Entries: ", picklistEntries);
						skuid.jQuery.each(picklistEntries, function(i, entry){
							excludePicklistProperties.push({
								id: "step_" + entry.label.replace(/\s/g, ''), //lose the spaces and make the label a unique ID
								type: "boolean",
								label: entry.label,
								onChange: function(){
									component.refresh();
						...
						//Push the new options into the wizard
							propCategories.push({
								name: "Basic",
								props: properties,
							},
							{
								name: "Exclude",
								props: excludePicklistProperties,
							});
							propertiesObj.applyPropsWithCategories(propCategories,state); //do the Apply here asyncronously
					});

				} else { //No Picklist to Render Exclude Property Page
