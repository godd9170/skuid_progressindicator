(function(skuid){
	skuid.componentType.register('progressindicator__progress_indicator', function(element, xmlDef, component){
		...
		if(mode == 'tabwiz'){
			...
			console.log("All Attrs: ", xmlDef.attr());
			$.each(picklistField.picklistEntries, function(i, entry){
				//rip through the builder attr's and get their exlusion booleans
				console.log("STEP: ", xmlDef.attr('step_' + i));
				var stepEntry = xmlDef.attr('step_' + entry.label.replace(/\s/g, '')); //spaceless label is the unique id
